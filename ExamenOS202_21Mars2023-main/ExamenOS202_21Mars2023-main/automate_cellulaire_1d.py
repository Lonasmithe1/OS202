...
rank = comm.Get_rank()
print(rank)
print("On a ",nombre_cas)
if rank == 0: # The master is the only process that reads the fileo
    data = nombre_cas # something read from file

# Divide the data among processes
#data = comm.scatter(data, root=0)
#(nombre_cas/nprocs)*rank,(nombre_cas/nprocs)*(rank+1)
#for num_config in range(int((nombre_cas/nprocs)*rank),int((nombre_cas/nprocs)*(rank+1))):
for num_config in range(rank,nombre_cas,nprocs):
...
print(f"Temps total : {temps_total:.6g}")

...
for r in range(nbre_repet):
    t1 = time.time()
    print("")
    nuage = np.array(np.array([[resolution_x * i * math.cos(48371.*i)/taille_nuage for i in range(rank,taille_nuage,nprocs)], [resolution_y * math.sin(50033./(i+1.)) for i in range(rank,taille_nuage,nprocs)]], dtype=np.float64).T)
    t2 = time.time()
...
if rank !=0:
    ...
    comm.send(enveloppe, dest=0)

# Si le rang n'est pas 1, recevoir le message envoyé par le processus 1
if rank == 0 :
    print("On va recevoir les tableaux")
    for i in range(nprocs):
        #enveloppe += comm.recv(source=i)
        enveloppe= np.concatenate((comm.recv(source=i), enveloppe),axis=0)
    print("On a recu les tableaux")
    
    enveloppe_enveloppe = calcul_enveloppe(enveloppe)
   ...
   # affichage du nuage :
    ...
    for i in range(len(enveloppe_enveloppe[:])-1):
        plt.plot([enveloppe_enveloppe[i,0],enveloppe_enveloppe[i+1,0]], [enveloppe_enveloppe[i,1], enveloppe_enveloppe[i+1,1]], 'bo', linestyle="-")
    plt.show()

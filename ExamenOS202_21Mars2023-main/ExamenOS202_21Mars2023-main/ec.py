...
for r in range(nbre_repet):
    t1 = time.time()
    print("")
    nuage = np.array(np.array([[resolution_x * i * math.cos(48371.*i)/taille_nuage for i in range(0,taille_nuage,2)], [resolution_y * math.sin(50033./(i+1.)) for i in range(0,taille_nuage,2)]], dtype=np.float64).T)
    nuage_2 = np.array(np.array([[resolution_x * i * math.cos(48371.*i)/taille_nuage for i in range(1,taille_nuage,2)], [resolution_y * math.sin(50033./(i+1.)) for i in range(1,taille_nuage,2)]], dtype=np.float64).T)
    ...
    enveloppe_2 = calcul_enveloppe(nuage_2)
    t2 = time.time()
    elapsed_convexhull += t2 - t1



    #enveloppe_fusion=enveloppe_2+enveloppe
    enveloppe_fusion = np.concatenate((enveloppe_2, enveloppe),axis=0)
    enveloppe_enveloppe = calcul_enveloppe(enveloppe_fusion)
    print(enveloppe)
    print(enveloppe_2)
    print(enveloppe_enveloppe)
